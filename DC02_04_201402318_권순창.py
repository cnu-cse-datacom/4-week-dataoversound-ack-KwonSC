...
def bitstreamGenerator(byte_stream, stream):
    bitstreama = byte_stream >> 4
    bitstreamb = byte_stream & 15
    stream.append((bitstreama*STEP_HZ)+START_HZ)
    stream.append((bitstreamb*STEP_HZ)+START_HZ)

def encode(byte_stream):
    bts = byte_stream.encode("utf-8")
    bts = RSCodec(FEC_BYTES).encode(bts)
    return bts
...
def play(bitstream):
    ...
    for i in range(len(bitstream)):
        print(bitstream[i])
        samples = (np.sin(2*np.pi*np.arange(44100*duration)*(bitstream[i]/44100))).astype(np.float32)
    ...
    while True:
        ...
        if in_packet and match(dom, HANDSHAKE_END_HZ):
            ...
            try:
                ...
                if bytearray(b'201402318') in byte_stream:
                    byte_stream = re.sub(b'201402318', b'', byte_stream)
                    byte_stream = byte_stream.decode("utf-8")
                    display(byte_stream)
                    byte_stream = encode(byte_stream)
                    bts = []
                    bts.append(HANDSHAKE_START_HZ)
                    for i in range(len(byte_stream)):
                        bitstreamGenerator(byte_stream[i],bts)
                    bts.append(HANDSHAKE_END_HZ)
                    play(bts)
